...
**Independent Test**: With a webhook URL configured and pointing to a request-capture tool (e.g., a test endpoint), triggering a scan produces a POST request whose body contains the site URL, the vulnerability status, the check timestamp, and the affected package list.

**Acceptance Scenarios**:

1. **Given** a webhook URL is configured and the site has no vulnerable packages, **When** a scan completes, **Then** a POST request is sent to the endpoint with status "Safe", an empty affected packages list, and the UTC timestamp of the completed scan.
2. **Given** a webhook URL is configured and the site has one or more vulnerable packages, **When** a scan completes, **Then** a POST request is sent to the endpoint with status "Vulnerable" and a list of each affected package with its name, installed version, advisory reference, and severity.
3. **Given** a webhook URL is configured, **When** the POST request is sent, **Then** the payload includes the primary URL of the site and the UTC timestamp of the completed scan.
...
- Q: What vocabulary should the payload use for the overall status? → A: Use the internal values "Vulnerable" and "Safe" to align with the codebase; no mapping layer needed.
- Q: Should the payload include the date and time of the check? → A: Yes — include the UTC timestamp of the completed scan.
...
- **FR-003a**: The POST payload MUST include the UTC date and time at which the vulnerability scan completed.
- **FR-004**: The POST payload MUST include the overall vulnerability status, expressed as either "Vulnerable" (one or more affected advisories found) or "Safe" (no affected advisories found).
...
- **Scan Notification Payload**: The data object sent to the webhook endpoint — site URL, vulnerability status ("Vulnerable" or "Safe"), UTC check timestamp, and the list of affected packages (each with name, installed version, advisory reference, and severity).
...
- **SC-004**: The POST payload received by the webhook endpoint contains all four required fields (site URL, status, check timestamp, affected packages) in a consistent, machine-readable format.
